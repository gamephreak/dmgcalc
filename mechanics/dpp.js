...
const NATURES = require('../data/natures').NATURES[DPP];
...
function damage(attacker, defender, move, field) {
  ...
  attacker.stats.spe = util.getFinalSpeed(defender, field.weather);
  ...
  switch (move.name) {
    ...
    const FACADE_STATUS = ['Paralyzed', 'Poisoned', 'Badly Poisoned', 'Burned'];
		if (FACADE_STATUS.indexOf(attacker.status) !== -1) {
			move.bp *= 2;
			desc.moveBP = move.bp;
		}
		break;
  ...
  desc.attackEVs = attacker.evs[attackStat] +
	...
	if (attackBoost === 0 || (isCritical && attackBoost < 0)) {
		...
		attack = util.getSimpleModifiedStat(rawAttack, attackBoost);
		desc.attackerAbility = attacker.ability;
		desc.attackBoost = attackBoost;
	} else {
		attack = util.getModifiedStat(rawAttack, attackBoost);
		desc.attackBoost = attackBoost;
	}
  ...
  desc.defenseEVs = defender.evs[defenseStat] +
	...
	if (defenseBoost === 0 || (isCritical && defenseBoost > 0)) {
		...
		defense = util.getSimpleModifiedStat(rawDefense, defenseBoost);
		desc.defenderAbility = defAbility;
		desc.defenseBoost = defenseBoost;
	} else {
		defense = util.getModifiedStat(rawDefense, defenseBoost);
		desc.defenseBoost = defenseBoost;
	}
  ...
  if ((field.weather === 'Sun' && move.type === 'Fire') ||
      (field.weather === 'Rain' && move.type === 'Water')) {
		...
		desc.weather = field.weather;
            ...
            (['Rain', 'Sand', 'Hail'].indexOf(field.weather) !== -1 &&
  ...
  if (isCritical) {
	...
	// NB: the random factor is applied between the LO mod and the STAB mod,
